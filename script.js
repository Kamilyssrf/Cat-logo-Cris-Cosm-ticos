...
async function carregarProdutos() {
  try {
    ...
    produtos = produtos.map(p => ({
      ...
      imagem: `${SUPABASE_URL}/storage/v1/object/public/produtos/${encodeURIComponent(p.imagem)}`
    }));
...
function renderCategories() {
  ...
  categories.innerHTML = cats.map(c =>
    `<button class="chip ${c === categoriaAtual ? 'active' : ''}" data-cat="${c}">${c}</button>`
  ).join('');

  categories.querySelectorAll('.chip').forEach(botao => {
  ...
  grid.innerHTML = filtrados.map(p => `
    ...
    <article class="card">
      ...
      <div class="info">
        ...
        <div class="price ${p.preco === '' ? 'consult' : ''}">
          ${money(p.preco)}
        </div>
